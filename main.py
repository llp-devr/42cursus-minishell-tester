...
class ShellPty:
    ...
    def exec(self, command):
        ...
        self.child.expect("\n")
        if len(command) > 57:
            output += f"crashinette>> {command[:43]}...\n"
        else:
    ...
    def close(self):
        ...
        print(f"Exit status: {self.child.exitstatus}")


def assert_equals(output1, output2):
    ...
    for l1, l2 in zip(lines1, lines2):
        if l1 == l2:
            color = Fore.GREEN
        elif l1.replace("minishell", "crashinette").replace(
            "bash", "crashinette"
        ) == l2.replace("minishell", "crashinette").replace("bash", "crashinette"):
            color = Fore.MAGENTA
        else:
            color = Fore.RED
            success = False
...
def main():
    ...
    minishell = ShellPty(cmd=args.minishell, args=[], prompt=args.minishell_prompt)
    ...
    with open(args.tests, "r") as file:
        ...
        for command in tests["commands"]:
            if not assert_equals(minishell.exec(command), bash.exec(command)):
                break
